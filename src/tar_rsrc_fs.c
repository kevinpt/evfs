...
Embedded Virtual Filesystem
  ...
  Tar resource VFS
  A VFS wrapper for accessing Tar resources as a filesystem

  Filesystem is initialized by passing an array of statically linked Tar
  file data. This can be generated by using "xxd -i" to convert a Tar file
  into a C header. You can also create an assembly wrapper that uses the
  .incbin directive or you can configure a linker script to link a tar file
  directly into its own section.

  File data can be accessed using the usual EVFS API with evfs_file_read().
  You can also directly access the resource data for a file by passing the
  EVFS_CMD_GET_RSRC_ADDR command to evfs_file_ctrl() and using evfs_file_size()
...
#if defined EVFS_USE_TARFS_SHARED_BUFFER && defined EVFS_USE_THREADING
...
static int tarfs__build_index(TarRsrcIterator *tar_it, EvfsTarIndex *ht) {
  if(!tar_rsrc_iter_begin(tar_it)) return EVFS_ERR;
  ...
  } while(tar_rsrc_iter_next(tar_it));
...
static int tarfs__file_ctrl(EvfsFile *fh, int cmd, void *arg) {
  ...
  TarfsData *fs_data = fil->fs_data;

  switch(cmd) {
    case EVFS_CMD_GET_RSRC_ADDR:
      {
        uint8_t **v = (uint8_t **)arg;
        *v = fs_data->resource + fil->header_offset + TAR_BLOCK_SIZE;
      }
...
static ptrdiff_t tarfs__file_read(EvfsFile *fh, void *buf, size_t size) {
  ...
  size_t remaining = fil->file_size - fil->read_pos;
  if(remaining <= 0) return 0;

  if(size > remaining)
    size = remaining;

  uint8_t *rsrc_pos = fs_data->resource + fil->header_offset + TAR_BLOCK_SIZE + fil->read_pos;
  fil->read_pos += size;
  memcpy(buf, rsrc_pos, size);
...
int evfs_register_tar_rsrc_fs(const char *vfs_name, uint8_t *resource, size_t resource_len,
  ...
  fs_data->resource = resource;
  fs_data->resource_len = resource_len;

  TarRsrcIterator tar_it;
  tar_rsrc_iter_init(&tar_it, resource, resource_len);
